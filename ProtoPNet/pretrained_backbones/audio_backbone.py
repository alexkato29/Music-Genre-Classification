...
class AudioNet(nn.Module):
    def __init__(self, num_classes, include_connected_layers=True):
        super(AudioNet, self).__init__()
        self.include_connected_layers = include_connected_layers
        ...
        if include_connected_layers:
            self.fc1 = nn.Linear(128 * 20, num_classes)  # Adjusted based on new conv/pool layers
    ...
    def forward(self, x):
        ...
        if self.include_connected_layers:
            x = x.view(x.size(0), -1)
            x = self.fc1(x)
            #x = F.relu(self.fc1(x))
            #x = self.dropout1(x)
